...
class NetworkManagerProtocol {
    ...
    func getCountry() {
        guard let url = URL(string: "https://gist.githubusercontent.com/peymano-wmt/32dcb892b06648910ddd40406e37fdab/raw/db25946fd77c5873b0303b858e861ce724e0dcd0/countries.json") else {
            self.delegate?.getData(result: .failure(.urlParseFailed))
            return
        }
        ...
        let dataTask = urlSession.dataTask(with: urlRequest) { data, urlResponse, error in
            ...
            guard let data = data else {
                self.delegate?.getData(result: .failure(.urlSessionFailed))
                return }
            
            do {
                let countries = try JSONDecoder().decode([Country].self, from: data)
                self.delegate?.getData(result: .success(countries))

            } catch {
                self.delegate?.getData(result: .failure(.jsonDecoderFailed))
            }
        }
        dataTask.resume()
    }
